...
static void	cub_move_ws(t_all *all, double step)
{
	...
	pos_x_temp3 = all->cam.px + all->cam.dx * step * 3;
	if (all->map.tab[(int)floor(all->cam.py)][(int)floor(pos_x_temp3)] != '1')
		all->cam.px = pos_x_temp;
	pos_y_temp = all->cam.py + all->cam.dy * step;
	pos_y_temp3 = all->cam.py + all->cam.dy * step * 3;
	if (all->map.tab[(int)floor(pos_y_temp3)][(int)floor(all->cam.px)] != '1')
...
static void	cub_move_ad(t_all *all, double step)
{
	...
	pos_x_temp3 = all->cam.px + all->cam.dy * step * 3;
	if (all->map.tab[(int)floor(all->cam.py)][(int)floor(pos_x_temp3)] != '1')
		all->cam.px = pos_x_temp;
	pos_y_temp = all->cam.py - all->cam.dx * step;
	pos_y_temp3 = all->cam.py - all->cam.dx * step * 3;
	if (all->map.tab[(int)floor(pos_y_temp3)][(int)floor(all->cam.px)] != '1')
		all->cam.py = pos_y_temp;
}
