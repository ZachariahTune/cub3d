...
//Вычисление углов обзора для каждой колонки пикселей на экране (луча)
...
//Трассировка луча до ближайшей вертикальной стены
...
//Трассировка луча до ближайшей горизонтальной стены (если ближе вертикальной)
...
static void	cub_draw_mlx(t_all *all)
{
	...
	while (all->ray.i < WIN_WIDTH)
	{
		...
		cub_trc_verwall(all);
		cub_trc_horwall(all);
		cub_draw_column(all, cub_wall_height(all), win_height);
		all->ray.i++;
	}
}

//Размещение кадра на экране
